...
import TrainingProviderSInformationDatatable from "./TrainingProviderSInformationDataTable";
...
import TrainingProviderSInformationFakerDialogComponent from "./TrainingProviderSInformationFakerDialogComponent";
import TrainingProviderSInformationSeederDialogComponent from "./TrainingProviderSInformationSeederDialogComponent";

const TrainingProviderSInformationPage = (props) => {
  ...
  const filename = "trainingProviderSInformation.csv";

  useEffect(() => {
    const _getSchema = async () => {
      const _schema = await props.getSchema("trainingProviderSInformation");
  ...
  useEffect(() => {
    ...
    client
      ...
      .catch((error) => {
        ...
        setLoading(false);
        props.alert({
          title: "Training Provider's Information",
          type: "error",
          message:
            error.message || "Failed get Training Provider's Information",
        });
      });
  ...
  const deleteAll = async () => {
    ...
    const promises = data.map((e) =>
      client.service("trainingProviderSInformation").remove(e._id),
    );
    await Promise.all(
      promises.map((p) =>
        p.catch((error) => {
  ...
  const onRowClick = ({ data }) => {
    navigate(`/trainingProviderSInformation/${data._id}`);
  };
    ...
    <div className="mt-5">
      <div className="grid">
        <div className="col-6 flex justify-content-start">
          <h3 className="mb-0 ml-2">Training Provider's Information </h3>
        </div>
        <div className="col-6 flex justify-content-end">
          <>
            <Button
              ...
              role="trainingProviderSInformation-add-button"
            />
            <SplitButton
      ...
      <div className="grid align-items-center">
        <div className="col-12" role="trainingProviderSInformation-datatable">
          <TrainingProviderSInformationDatatable
            items={data}
            fields={fields}
      ...
      <AreYouSureDialog
        ...
        onYes={() => deleteRow()}
      />
      <TrainingProviderSInformationEditDialogComponent
        entity={_.find(data, { _id: selectedEntityIndex })}
        show={showEditDialog}
        onHide={() => setShowEditDialog(false)}
        onEditResult={onEditResult}
      />
      <TrainingProviderSInformationCreateDialogComponent
        entity={newRecord}
        show={showCreateDialog}
        onHide={() => setShowCreateDialog(false)}
        onCreateResult={onCreateResult}
      />
      <TrainingProviderSInformationFakerDialogComponent
        show={showFakerDialog}
        onHide={() => setShowFakerDialog(false)}
        onFakerCreateResults={onFakerCreateResults}
      />
      <TrainingProviderSInformationSeederDialogComponent
...
export default connect(mapState, mapDispatch)(TrainingProviderSInformationPage);
