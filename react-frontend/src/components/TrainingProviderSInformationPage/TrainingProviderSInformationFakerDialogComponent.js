...
import trainingProviderSInformationFakerFactory from "./trainingProviderSInformationFakerFactory";
...
const TrainingProviderSInformationFakerDialogComponent = (props) => {
  ...
  const onRun = async () => {
    let fakeData = trainingProviderSInformationFakerFactory(
      props.user,
      fakerCount,
    );
    ...
    const promises = fakeData.map((elem) =>
      client.service("trainingProviderSInformation").create(elem),
    );
