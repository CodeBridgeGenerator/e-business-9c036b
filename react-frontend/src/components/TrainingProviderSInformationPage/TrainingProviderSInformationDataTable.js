...
const TrainingProviderSInformationDataTable = ({
  items,
  fields,
  onEditRow,
  onRowDelete,
  ...
  const pTemplate1 = (rowData, { rowIndex }) => (
    <p>{rowData.organizationAddress}</p>
  );
  const pTemplate2 = (rowData, { rowIndex }) => (
    <p>{rowData.organizationContactNumber}</p>
  );
  ...
  const paginatorRight = DownloadCSV({
    data: items,
    fileName: "trainingProviderSInformation",
  });
  const exportCSV = () => {
    ...
    <>
      ...
      >
        ...
        <Column
          ...
          body={pTemplate1}
          filter={selectedFilterFields.includes("organizationAddress")}
          hidden={selectedHideFields?.includes("organizationAddress")}
          sortable
          style={{ minWidth: "8rem" }}
        />
        <Column
          field="organizationContactNumber"
          header="Organization Contact Number"
          body={pTemplate2}
          filter={selectedFilterFields.includes("organizationContactNumber")}
          hidden={selectedHideFields?.includes("organizationContactNumber")}
          sortable
          style={{ minWidth: "8rem" }}
        />
      ...
      <Dialog
        header="Upload TrainingProviderSInformation Data"
        visible={showUpload}
      ...
      <Dialog
        header="Search TrainingProviderSInformation"
