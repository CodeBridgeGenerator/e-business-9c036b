...
const TrainingProviderSInformationDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const pTemplate2 = (rowData, { rowIndex }) => <p >{rowData.organizationContactNumber}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "trainingProviderSInformation"});
...
<Column field="organizationContactNumber" header="Organization Contact Number" body={pTemplate2} filter={selectedFilterFields.includes("organizationContactNumber")} hidden={selectedHideFields?.includes("organizationContactNumber")}  sortable style={{ minWidth: "8rem" }} />
            <Column header="Edit" body={editTemplate} />
            <Column header="Delete" body={deleteTemplate} />
            
        </DataTable>
        <Dialog header="Upload TrainingProviderSInformation Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService 
          user={user} 
      ...
      <Dialog header="Search TrainingProviderSInformation" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default TrainingProviderSInformationDataTable;