...
const OfficerInChargeCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            officerName: _entity?.officerName,designation: _entity?.designation,telephoneNumber: _entity?.telephoneNumber,icPassportNumber: _entity?.icPassportNumber,email: _entity?.email,organizationName: _entity?.organizationName,
