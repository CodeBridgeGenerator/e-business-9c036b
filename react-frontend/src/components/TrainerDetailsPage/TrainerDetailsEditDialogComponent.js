...
nationality: _entity?.nationality,
        ...
        const result = await client.service("trainerDetails").patch(_entity._id, _data);
        props.onHide();
        props.alert({ type: "success", title: "Edit info", message: "Info trainerDetails updated successfully" });
    ...
    return (
        <Dialog header="Edit Trainer Details" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="trainerDetails-edit-dialog-component">
                <div className="col-12 md:col-6 field mt-5">
        <span className="align-items-center">
            <label htmlFor="trainerIcPassportNumber">Trainer IC/Passport Number:</label>
            <InputText id="trainerIcPassportNumber" className="w-full mb-3 p-inputtext-sm" value={_entity?.trainerIcPassportNumber} onChange={(e) => setValByKey("trainerIcPassportNumber", e.target.value)}  />
        </span>
        </div>
<div className="col-12 md:col-6 field mt-5">
        <span className="align-items-center">
